import os


def get_files_info(working_directory: str, directory: str = ".") -> str:
    try:
        abs_working_dir = os.path.abspath(working_directory)
        target_dir = os.path.normpath(os.path.join(abs_working_dir, directory))
        if os.path.commonpath([abs_working_dir, target_dir]) != abs_working_dir:
            return f'Error: Cannot list "{directory}" as it is outside the permitted working directory'
        if not os.path.isdir(target_dir):
            return f'Error: "{directory}" is not a directory'
        files_info: list[str] = []
        for filename in os.listdir(target_dir):
            filepath = os.path.join(target_dir, filename)
            is_dir = os.path.isdir(filepath)
            file_size = os.path.getsize(filepath)
            files_info.append(
                f"- {filename}: file_size={file_size} bytes, is_dir={is_dir}"
            )
        return "\n".join(files_info)
    except Exception as e:
        return f"Error listing files: {e}"

# import os

# def get_files_info(working_directory: str, directory: str = ".") -> str:
#     working_dir_abs, target_dir = "", ""
    
#     try:
#         working_dir_abs = os.path.abspath(working_directory)
#     except Exception as e:
#         return f'Error: Failed to resolve working directory path - {e}'
    
#     try:
#         target_dir = os.path.normpath(os.path.join(working_dir_abs, directory))
#     except Exception as e:
#         return f'Error: Failed to resolve target directory path - {e}'

#     if not os.path.commonpath([working_dir_abs, target_dir]) == working_dir_abs:
#         return f'Error: Cannot list "{target_dir}" as it is outside the permitted working directory'
    
#     if not os.path.isdir(target_dir):
#         return f'Error: "{target_dir}" is not a directory'
    
#     else:
#         items_list = []
#         try: 
#             items_list = os.listdir(target_dir)
#         except Exception as e:
#             return f'Error: Failed to list directory "{target_dir}" - {e}'
        
#         info_string = ""
#         for item in items_list:
#             item_path = ""
#             size:int = 0
            
#             try:
#                 size = os.path.getsize(item)
#             except Exception as e:
#                 return f'Error: Failed to get size for "{item}" - {e}'
            
#             info_string += f"{item}: file_size={size} bytes is_dir={os.path.isdir(item)}\n"
#         return info_string

